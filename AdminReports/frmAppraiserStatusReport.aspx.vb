...
Partial Class AdminReports_frmAppraiserStatusReport
...
#End Region
    ...
    Private Sub BindData(ByVal CycleId)
        ...
        arrPara(1, 0) = CycleId
        arrPara(1, 1) = 0
        'dsReport = objComm.RunSP("SpGetApsrApseRep", arrPara)
        dsReport = objADO.RunSPDS("SpGetApsrApseRep", arrPara)


        ' dg_DisplayReport.CurrentPageIndex = 0
        'Response.Write(dg_DisplayReport.PageCount)
        dg_DisplayReport.PageSize = 40
    ...
    Private Sub dg_DisplayReport_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_DisplayReport.ItemCommand
        If e.CommandName = "cmd_ViewDetails" Then
            ...
            scriptString = "<script>window.open('frmAppraseDetails.aspx?CycleApsrID=" & e.CommandArgument & "','AppraseReports','menubar=0,toolbar=0,resizable=1,left=0,top=0,width=1015,height=690,resizable=1,status=1,scrollbars=1')"   ' ,'menubar=0,toolbar=0,resizable=1,left=0,top=0,width=1024,height=768'
            scriptString += ";</" + "script>"
    ...
    Private Sub LinkButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LinkButton1.Click
        ...
        ReDim arrPara(0, 1)
        arrPara(0, 0) = ViewState("AssmntcycleId")
        arrPara(0, 1) = 1
        ...
        objDr = objADO.RunSP("SpGetApsrApseRep", arrPara, 0, objCon, objCom)
        ...
        While objDr.Read
            oSheet.Range("A" & ctr).Value = objDr.Item("ApsrDescr")
            oSheet.Range("B" & ctr).Value = objDr.Item("ApseTot")
            oSheet.Range("C" & ctr).Value = objDr.Item("ApseDone")
            oSheet.Range("D" & ctr).Value = objDr.Item("Apsebalance")
            ctr = ctr + 1
        End While

        If File.Exists(System.Configuration.ConfigurationSettings.AppSettings("ExcelPath") & "\" & "ExcelReportsByAppraiser" & ".xls") Then
            File.Delete(System.Configuration.ConfigurationSettings.AppSettings("ExcelPath") & "\" & "ExcelReportsByAppraiser" & ".xls")
        End If
        oBook.SaveAs(System.Configuration.ConfigurationSettings.AppSettings("ExcelPath") & "\" & "ExcelReportsByAppraiser" & ".xls")
        oBook.Close()
        oExcel.Quit()
    ...
    Function fnRetunPathForDownload()
        Dim path As String = ConfigurationSettings.AppSettings("ExcelPath") & "\ExcelReportsByAppraiser.xls"
        'Response.Redirect(Path)
        'Return Path
        Response.Clear()
        Response.ContentType = "application/vnd.ms-excel"
        Response.AddHeader("Content-Disposition", "attachment;filename=ExcelReportsByAppraiser.xls")
