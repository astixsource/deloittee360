...
public partial class frmSendEmailInvite : System.Web.UI.Page
{
    ...
    public static string fngetdata(int CycleId, string UserType)
    {
        ...
        try
        {
            ...
            List<SqlParameter> sp = new List<SqlParameter>()
              {
                   ...
                   new SqlParameter("@MailType", 4), //  360 Degree Feedback for your stakeholders: Survey Launch // Mail Type 4
              };
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    string RaterId = Ds.Tables[0].Rows[i]["RaterId"].ToString();
                    string RaterName = Ds.Tables[0].Rows[i]["RaterName"].ToString();
                    string RaterEMailId = Ds.Tables[0].Rows[i]["RaterEMailId"].ToString();
    ...
    public static string fnSave(object udt_DataSaving)
    {
        ...
        try
        {
            ...
            foreach (DataRow drow in dtDataSaving.Rows)
            {
                try
                {
                    string RaterId = drow["RaterId"].ToString();
                    string RaterName = drow["RaterName"].ToString();
                    string RaterEMailId = drow["RaterEMailId"].ToString();
                    ...
                    if (strStatus == "1")
                    {
                        fnUpdateMailSp("spMailUpdateLog", RaterId, "1", "2", "1", Scon);
                    }
    ...
    public static string fnSendICSFIleToUsers(string RaterId, string RaterName, string RaterEMailId, string RaterUserName, string RaterPassword, string DeadlineDate)
    {
        ...
        try
        {
            ...
            msg.Subject = "360 Degree Feedback for your stakeholders: Survey Launch";
            ...
            strBody.Append("<p>Selection: You have been selected as one of the rater for the 360-Degree Feedback process. Your participation has been approved by respective stakeholder's Reporting Manager/Coach.</p>");
    ...
    public static string fnSendICSFIleToUsers_EXT(string RaterId, string RaterName, string RaterEMailId, string RaterUserName, string RaterPassword, string DeadlineDate)
    {
        ...
        try
        {
            ...
            strBody.Append("<p>Selection: You have been selected as one of the raters for the 360-Degree Feedback process. Your participation has been approved by respective stakeholder's Reporting Manager/Coach.</p>");
            strBody.Append("<p>Feedback survey: The feedback survey, tailored to rate various competencies aligned with our Deloitte Future Leader Framework (DFLF) can be accessed here: <a href='https://apcdeloitte.sharepoint.com/sites/in/psupport/hr/Pages/360-Degree-Framework-Toolkit.aspx'>360 Feedback Toolkit </a></p>");
            strBody.Append("<p><b>Your role:</b></p>");
            strBody.Append("<p>Your feedback, based on your experience working with the above mentioned practitioners, will contribute significantly to their professional growth.</p>");
            strBody.Append("<p>Please provide candid and constructive feedback to ensure a well-rounded assessment.</p>");
            ...
            strBody.Append("<p><b>Email: " + RaterUserName + "</b></p>");
            ...
            strBody.Append("<p>Note: This is a system-generated email. Please do not reply to this ID.s</p>");
    ...
    public static void fnUpdateMailSp(string SPName, string EmpNodeID, string FlgMailState, string flgUpdate, string UserType, SqlConnection Scon1)
    {
        ...
        Scmd.Parameters.AddWithValue("@MailType", 4);
