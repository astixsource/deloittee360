...
public partial class AdminReports_frmNominationStatusReport : System.Web.UI.Page
{
    ...
    {
        SqlConnection con = new SqlConnection(strCon.Split('|')[0]);
        string com = "spGetstatusListForSurveyCompletion";
        SqlDataAdapter adpt = new SqlDataAdapter(com, con);
    ...
    public static string GetDetails(string CycleId, string StatusID)
    {
        try
        {
            ...
            Scmd.CommandText = "spGetCompletionStatus";
            ...
            SkipColumn[0] = "Participant Level";
    ...
    private static string dttoHTML(DataTable dt,  string[] SkipColumn, string lbl)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (dt.Rows[i]["flgAllowOpenSurvey"].ToString() == "1")
            {
                sb.Append("<td style='width:12%'><a href = '###' onclick='fnOpenSurvey(\"" + dt.Rows[i]["Participant Name"].ToString().Trim() + "\",\"" + dt.Rows[i]["Rater Name"].ToString().Trim() + "\"," + dt.Rows[i]["RspID"].ToString() + ", this);' style='margin-left: 20px;'>Open Survey</a></td>");
            }
            if (dt.Rows[i]["flgAllowOpenSurvey"].ToString() != "1")
            {
                sb.Append("<td></td>");
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        ...
        Scmd.Parameters.AddWithValue("@CycleId", ddlCycle.SelectedValue);
         Scmd.Parameters.AddWithValue("@StatusId", ddlStatus.SelectedValue);
        Scmd.CommandType = CommandType.StoredProcedure;
        Scmd.CommandTimeout = 0;
        SqlDataAdapter Sdap = new SqlDataAdapter(Scmd);
        DataSet ds = new DataSet();
        ...
        try
        {
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=StatusReport_" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xlsx");
    ...
    [System.Web.Services.WebMethod()]
    public static string fnReOpen_Result(int RspID, int CycleID , int LoginID)
    {
        ...
        try
        {
            ...
            {
                using (SqlCommand command = new SqlCommand("spManage_ReOpenSurveyCompletion", Scon))
                {
